...
test('@claim:canvas-authoring adds and arranges connected visual items', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'desktop', 'one pointer-authoring run is enough')
  await page.goto('/demo')
  await page.locator('[data-add="card"]').first().click()
  await page.locator('#edit-node-label').fill('New source')
  await page.locator('#edit-node-label').blur()
  await page.locator('[data-add="number"]').first().click()
  await expect(page.locator('.canvas-node')).toHaveCount(6)
  await page.getByRole('button', { name: /Arrow/ }).click()
  await page.locator('#arrow-from').selectOption('left')
  await page.locator('#arrow-to').selectOption('right')
  await page.locator('#arrow-label').fill('keeps the total')
  await page.getByRole('button', { name: 'Add arrow' }).click()
  await expect(page.locator('.arrow-path')).toHaveCount(2)

  const card = page.locator('[data-select-node="left"]')
  const before = Number(await page.locator('.arrow-path').first().getAttribute('x1'))
  const box = await card.boundingBox()
  expect(box).not.toBeNull()
  await page.mouse.move((box?.x ?? 0) + (box?.width ?? 0) / 2, (box?.y ?? 0) + (box?.height ?? 0) / 2)
  await page.mouse.down()
  await page.mouse.move((box?.x ?? 0) + (box?.width ?? 0) / 2 + 48, (box?.y ?? 0) + (box?.height ?? 0) / 2)
  await page.mouse.up()
  const after = Number(await page.locator('.arrow-path').first().getAttribute('x1'))
...
test('@claim:timed-intervals shows and exports reconciled timing', async ({ page }) => {
  ...
  await expect(page.locator('.timeline')).toContainText('2.0–4.0 s')
  await end.fill('0')
  await end.press('Tab')
